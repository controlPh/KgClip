...
.\.venv\Scripts\python -c "from pathlib import Path; compile(Path(r'app.py').read_text(encoding='utf-8'), 'app.py', 'exec'); print('APP_SYNTAX_OK')"
if errorlevel 1 (
    echo.
...
start "KG Scene Retrieval App" /min cmd /d /c "cd /d ""%CD%"" && .\.venv\Scripts\python app.py"
if errorlevel 1 (
