...
def evaluate_strategy(
    ...
    for row_index, row in dataframe.iterrows():
        ...
        run = run_fn(query_text, frame_search_limit=frame_search_limit, map_depth=map_depth, k=k)
...
def print_summary_table(summaries: list[dict], k: int) -> None:
    ...
    print()
    print(
        f"Metrics reported: Precision@{k}, Recall@{k}, mAP, ConstraintConsistency@{k}, average response time"
    )
