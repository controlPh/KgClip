...
def resolve_frame_path(raw_path: str | Path | None) -> Path | None:
    ...
    if parts:
        if parts[0] in {"samples", "sweeps"}:
            for blob_root in blob_roots:
                ...
                # If path starts with "samples", also try "sweeps"
                if parts[0] == "samples":
                    sweeps_parts = ("sweeps",) + parts[1:]
                    candidates.append(blob_root.joinpath(*sweeps_parts))
                # If path starts with "sweeps", also try "samples"
                elif parts[0] == "sweeps":
                    samples_parts = ("samples",) + parts[1:]
                    candidates.append(blob_root.joinpath(*samples_parts))
    ...
    if metadata:
        for blob_root in blob_roots:
            ...
            # Also try swapping samples/sweeps in metadata filename
            metadata_filename = metadata["filename"]
            if metadata_filename.startswith("samples/"):
                sweeps_filename = metadata_filename.replace("samples/", "sweeps/", 1)
                candidates.append(blob_root / sweeps_filename)
            elif metadata_filename.startswith("sweeps/"):
                samples_filename = metadata_filename.replace("sweeps/", "samples/", 1)
                candidates.append(blob_root / samples_filename)
...
def _text_features_to_tensor(raw_features: object) -> torch.Tensor:
    """`transformers` 新版本里 `get_text_features` 可能返回张量或 `ModelOutput`，统一成 `(batch, dim)`。"""
    if isinstance(raw_features, torch.Tensor):
        return raw_features
    pooler = getattr(raw_features, "pooler_output", None)
    if isinstance(pooler, torch.Tensor):
        return pooler
    text_embeds = getattr(raw_features, "text_embeds", None)
    if isinstance(text_embeds, torch.Tensor):
        return text_embeds
    raise TypeError(
        f"无法从 get_text_features 的返回值取出向量：{type(raw_features)!r}（需要 Tensor 或含 pooler_output/text_embeds 的输出）。"
    )
...
def encode_text_query(text: str) -> tuple[np.ndarray, str]:
    ...
    with torch.inference_mode():
        raw_features = model.get_text_features(**encoded_inputs)
        features = _text_features_to_tensor(raw_features)
...
def enrich_hit_record(record: dict) -> dict:
    # Get frame_path from Milvus record, or fallback to ID_TO_RAW_PATH
    frame_path = record.get("frame_path")
    if not frame_path:  # Handle None or empty string
        frame_path = ID_TO_RAW_PATH.get(record["id"], "")
    raw_path = str(frame_path or "")
    ...
    for key, value in metadata.items():
        # Always update these fields regardless of whether they exist
        if key in ["raw_frame_path", "resolved_frame_path"]:
            enriched_record[key] = value
            continue
        # For other fields, only update if they don't exist
...
/* 仅供 JS 触发 Gradio 关闭事件，不参与布局与点击命中（.click() 仍有效） */
.lightbox-dismiss-proxy {
    position: fixed !important;
    width: 1px !important;
    ...
    with gr.Column(elem_id="app-shell", elem_classes="app-shell"):
        ...
        with gr.Column(elem_classes="results-shell"):
            ...
            with gr.Column(visible=True, elem_id="image_result_zone") as image_result_zone:
                ...
                lightbox_dismiss_proxy = gr.Button(
                    "关闭灯箱",
                    elem_id="lightbox-dismiss-proxy",
                    elem_classes=["lightbox-dismiss-proxy"],
                )
...
<script>
(function () {
  function lightboxShellOpen() {
    var lb = document.getElementById("image-lightbox");
    if (!lb) return false;
    var st = window.getComputedStyle(lb);
    if (st.display === "none" || st.visibility === "hidden") return false;
    return lb.offsetParent !== null;
  }
  function clickLightboxDismissProxy() {
    var root = document.getElementById("lightbox-dismiss-proxy");
    if (!root) return;
  ...
  document.addEventListener(
    ...
    function (e) {
      ...
      if (!lightboxShellOpen()) return;
      e.preventDefault();
      clickLightboxDismissProxy();
    },
    true,
  );
  document.addEventListener(
    ...
    function (e) {
      ...
      var card = lb.querySelector(".image-lightbox-card");
    ...
    # 在 Gradio 5 中，与 fn 同时传入的 js 先于 fn 执行且需返回与 outputs 等长的值；错误的返回值会打乱状态。
    # 这里仅在 Python 更新组件之后用 .then() 切换 body 滚动锁，避免关不掉/页面无法滚动。
    OPEN_LIGHTBOX_SCROLL_LOCK_JS = """
    () => {
        document.body.classList.add('lightbox-open');
        document.documentElement.classList.add('lightbox-open');
    }
    """
    CLOSE_LIGHTBOX_SCROLL_LOCK_JS = """
    () => {
    ...
    def dynamic_retrieve(text: str, mode: str):
        ...
        try:
            ...
            return build_output_values(
                result_notice=f"<div class='result-notice' style='color:#b42318'>{escape(str(exc))}</div>",
                explanation=build_preview_explanation(query, mode),
            )
    ...
    for preview_button, image_component, image_caption in zip(image_preview_buttons, image_outputs, image_caption_outputs):
        ...
        _preview_dep.then(
            ...
            inputs=None,
            outputs=None,
            js=OPEN_LIGHTBOX_SCROLL_LOCK_JS,
            show_api=False,
            queue=False,
        )
    _close_lightbox_dep = image_lightbox_close.click(fn=close_image_preview, outputs=image_preview_outputs)
    _close_lightbox_dep.then(
        fn=None,
        inputs=None,
        outputs=None,
        js=CLOSE_LIGHTBOX_SCROLL_LOCK_JS,
    ...
    _proxy_close_dep = lightbox_dismiss_proxy.click(fn=close_image_preview, outputs=image_preview_outputs)
    _proxy_close_dep.then(
        fn=None,
        inputs=None,
        outputs=None,
        js=CLOSE_LIGHTBOX_SCROLL_LOCK_JS,
...
def launch_interface() -> None:
    # 默认开启 Gradio 临时公网链接；显式设置 KGSR_ENABLE_SHARE=0 / false / off 可关闭
    share_enabled = env_flag("KGSR_ENABLE_SHARE", default=True)
    ...
    try:
        ...
        print(f"\n[Gradio] 本地访问: {local_url}")
        if share_enabled:
            if share_url:
                print(f"[Gradio] 公网链接(临时): {share_url}\n")
            else:
                print(
                    "[Gradio] 公网链接: 未返回 URL（可检查本机网络、代理或 gradio 隧道服务）。\n"
                )
        else:
            print("[Gradio] 公网链接: 已通过 KGSR_ENABLE_SHARE 关闭。\n")
    except Exception as exc:
